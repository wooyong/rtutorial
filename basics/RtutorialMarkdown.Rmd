...
RStudio will create a skeleton file that contains some basic explanations. Read those.
...
- If you know LaTeX, you can use LaTeX in R markdown.

- R markdown understands LaTeX syntax (at least those in the math mode) --- just write them.

<!-- if you want to use LaTeX packages in R markdown, include them in the "header-includes" declaration in the header as in this document -->

- Below I write down LaTeX codes to write math symbols and equations. 
...
This is handy for homework submission because you don't need to separately submit the code or copy the R output into the document.
...
R markdown offers options that can configure how the R chunks are displayed and executed. For example, you can decide whether to hide the code, whether to hide the result, whether to display the error message, etc.

For a list of such options, check **5. Embed code** of the R markdown cheatsheet.

As an example, we will take a look at the `eval=FALSE` option, which tells R to not execute the code.
